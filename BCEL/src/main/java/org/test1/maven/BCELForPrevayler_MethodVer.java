...
public class BCELForPrevayler_MethodVer {
	static String target = "C:\\Users\\user\\Downloads\\eclipse-java-mars-2-win32-x86_64\\Prevayler_P1\\bin";
	...
	//	        System.err.println("Could not get class ");
		    ...
		    for (int i = 0; i < methods.length; i++) {
		    	...
		    	}else if(name.contains("ESTest") && !name.contains("scaffolding") && !methods[i].getName().equals("<init>") && !methods[i].getName().equals("initEvoSuiteFramework") && !methods[i].getName().equals("exitEvoSuiteFramework") && !methods[i].getName().equals("clearEvoSuiteFramework")){
		    		MethodGen mgen = new MethodGen(methods[i], name + ".class", cp);
		    		InstructionList ilist = mgen.getInstructionList();
