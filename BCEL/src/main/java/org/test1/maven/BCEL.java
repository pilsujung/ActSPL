...
public class BCEL {
	...
	//	        System.err.println("Could not get class ");
		    ...
		    for (int i = 0; i < methods.length; i++) {
		    	if(methods[i].getName().equals("initEvoSuiteFramework") && name.contains("scaffolding")){
		    		...
		    		//System.out.println("initEvosuiteFramework -> CoverageInfo.save(" + modClass.getClassName().substring(modClass.getClassName().lastIndexOf(".")+1) + ")");
...
//		    		Iterator<InstructionHandle []> it = f.search("invokespecial");
		    		
		    		InstructionHandle [] ihs = ilist.getInstructionHandles();
//		    		ihs[0] = ilist.append(ihs[0], new GETSTATIC(cp.addFieldref("java.lang.System", "out", "Ljava/io/PrintStream;")));
//		    		ihs[0] = ilist.append(ihs[0], new LDC(cp.addString(modClass.getClassName().substring(modClass.getClassName().lastIndexOf(".")+1))));
//		    		ihs[0] = ilist.append(ihs[0], new INVOKEVIRTUAL(cp.addMethodref("java.io.PrintStream", "println", "(Ljava/lang/String;)V")));
//		    		
		    		ihs[0] = ilist.append(ihs[0], new LDC(cp.addString(path)));
		    		ihs[0] = ilist.append(ihs[0], new INVOKESTATIC(cp.addMethodref("CoverageInfo", "save", "(Ljava/lang/String;)V")));
		    		
		    		//System.out.println("<init> -> CoverageInfo.save(" + modClass.getClassName().substring(modClass.getClassName().lastIndexOf(".")+1) + ")");
		    		
		    		
		    		
		    		/*if(it.hasNext()){
		    			InstructionHandle [] matches = it.next();
		    			matches[0] = ilist.append(matches[0], new GETSTATIC(cp.addFieldref("java.lang.System", "out", "Ljava/io/PrintStream;")));
		    			matches[0] = ilist.append(matches[0], new LDC(cp.addString(" You are a real geek!")));
		    			matches[0] = ilist.append(matches[0], new INVOKEVIRTUAL(cp.addMethodref("java.io.PrintStream", "println", "(Ljava/lang/String;)V")));
		    		}*/
