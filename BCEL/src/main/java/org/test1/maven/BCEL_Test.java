...
public class BCEL_Test {
	static String target = "C:\\Users\\user\\Desktop\\eclipse-java-mars-2-win32-x86_64\\TEST\\bin";
	...
	//	        System.err.println("Could not get class ");
		    ...
		    for (int i = 0; i < methods.length; i++) {
		    	if(methods[i].getName().equals("exitEvoSuiteFramework")){
			    	...
			    	int index = cp.addMethodref("CoverageInfo", "print", "()V");
			    	
		    		ilist.append(new INVOKESTATIC(index));
		            ...
		            ilist.dispose();
		    	}/*else if(!name.equals("CoverageInfo") && !methods[i].getName().equals("<clinit>") && !methods[i].getName().equals("<init>") && !methods[i].getName().equals("initEvoSuiteFramework")){
		    		MethodGen mgen = new MethodGen(methods[i], name + ".class", cp);
		    		InstructionList ilist = mgen.getInstructionList();
